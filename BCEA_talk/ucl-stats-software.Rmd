...
params: 
   conference: "UCL statistical software writers workshop"
   short_conference: "UCL stats software workshop"
...
- But it is also under constant development in the [GitHub repository](https://github.com/giabaio/BCEA)
   ...
   - Current *development* version: 2.4-1 (23 November 2021) &ndash; **major** update (soon to be released!)
...
```{r howwork5,echo=FALSE,R.options=list(width=160)}
inp$mat %>% as_tibble()
```
]
...
```{r howwork10, R.options=list(width=100)}
names(m)
```
